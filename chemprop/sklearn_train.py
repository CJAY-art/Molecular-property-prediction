...
def run_sklearn(args: SklearnTrainArgs,
    ...
    debug('Loading data')
    train_data = get_data(path=args.data_path,
                          args=args,
    ...
    if args.model_type == 'svm' and train_data.num_tasks() != 1:
        raise ValueError(f'SVM can only handle single-task data but found {train_data.num_tasks()} tasks')
    ...
    debug(f'Total size = {len(train_data):,} | train size = {len(train_data):,} | test size = {len(test_data):,}')
...
def sklearn_train() -> None:
    ...
    scores=run_sklearn(args=SklearnTrainArgs().parse_args())
    print(scores)