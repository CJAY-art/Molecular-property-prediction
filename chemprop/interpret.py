...
class ChempropModel:
    ...
    def __call__(self, smiles: List[str], batch_size: int = 500) -> List[List[float]]:
        ...
        test_data_loader = MoleculeDataLoader(dataset=test_data, batch_size=batch_size,drop_last=False)
...
def interpret(args: InterpretArgs) -> None:
    ...
    scores_df = pd.DataFrame(columns = ['smiles',property_name, 'rationale','rationale_score'])



    for smiles in all_smiles:
        score = scoring_function([smiles])[0]
        if score > 0.8:#args.prop_delta
            rationales = mcts(
                smiles=smiles[0],
        ...
        if len(rationales) == 0:
            ...
            scores_df=scores_df.append({'smiles':smiles,property_name:score},ignore_index=True)
        else:
            ...
            scores_df=scores_df.append({'smiles': smiles, property_name: score, 'rationale': rats[0].smiles, 'rationale_score': rats[0].P},ignore_index=True)

    scores_df.to_csv('interpret.csv')
