...
from chemprop.draw_pic import draw_curve

def ZW6(mol):
    return AllChem.GetMorganFingerprintAsBitVect(mol,3,nBits=256).ToBitString()
def get_mol(smile):
        mol = [AllChem.MolFromSmiles(s[0]) for s in smile]
        return mol
...
def xgboost_cv(max_depth_numbers,learning_rate_numbers,min_child_weight_numbers,
    ...
    pre.append([i[0] for i in test_preds])

    max_depth,learning_rate,min_child_weight,best_auc,sn,sp,Acc=0,0,0,0,0,0,0
        ...
        for learning_rate_number in learning_rate_numbers:
            for min_child_weight_number in min_child_weight_numbers:
                xgb_gbc = xgb.XGBClassifier(base_score=0.5, booster='gbtree', colsample_bylevel=1,
                                            ...
                                            max_depth=max_depth_number, min_child_weight=min_child_weight_number, n_estimators=2000,
                ...
                pre_pro1 = xgb_gbc.predict_proba(test_feature)[:, 1]
                fpr, tpr, threshold = roc_curve(test_target, pre_pro1)
                AUC = auc(fpr, tpr)
                pre_pro2 = [1 if i > 0.5 else 0 for i in pre_pro1]
                tn, fp, fn, tp = confusion_matrix(test_target, pre_pro2).ravel()
                Sn = tp / (tp + fn)
                Sp = tn / (tn + fp)
                acc = accuracy_score(test_target, pre_pro2)
                if AUC>best_auc:
                    max_depth=max_depth_number
                    learning_rate=learning_rate_number
                    min_child_weight=min_child_weight_number
                    best_auc=AUC
                    sn=Sn
                    sp=Sp
                    Acc=acc
    ...
    xgb_type = 'dhtnn+xgb'
    pre.append(pre_list)
    scores.append([xgb_type,max_depth,learning_rate,min_child_weight,best_auc,sn,sp,Acc])
        ...
        for learning_rate_number in learning_rate_numbers:
            for min_child_weight_number in min_child_weight_numbers:
                ...
                xgb_gbc.fit(train_morgan_feature, train_target, eval_set=[(val_morgan_feature, val_target)],
                            eval_metric='auc',
                            early_stopping_rounds=200)
                pre_pro1 = xgb_gbc.predict_proba(test_morgan_feature)[:, 1]
                fpr, tpr, threshold = roc_curve(test_target, pre_pro1)
                AUC = auc(fpr, tpr)
                pre_pro2 = [1 if i > 0.5 else 0 for i in pre_pro1]
                tn, fp, fn, tp = confusion_matrix(test_target, pre_pro2).ravel()
        ...
        for learning_rate_number in learning_rate_numbers:
            for min_child_weight_number in min_child_weight_numbers:
                ...
                xgb_gbc.fit(train_gcn_mor_feature, train_target, eval_set=[(val_gcn_mor_feature, val_target)],
                            eval_metric='auc',
                            early_stopping_rounds=200)
                pre_pro1 = xgb_gbc.predict_proba(test_gcn_mor_feature)[:, 1]
                fpr, tpr, threshold = roc_curve(test_target, pre_pro1)
                AUC = auc(fpr, tpr)
                pre_pro2 = [1 if i > 0.5 else 0 for i in pre_pro1]
                tn, fp, fn, tp = confusion_matrix(test_target, pre_pro2).ravel()
                ...
                if AUC>best_auc:
                    ...
                    Acc=acc
                    pre_list=pre_pro1
    xgb_type = 'dhtnn+morgan+xgb'
    pre.append(pre_list)
    scores.append([xgb_type, max_depth, learning_rate, min_child_weight, best_auc, sn, sp, Acc])

    draw_curve(pre,test_target)
    scores_df = pd.DataFrame(scores)
    return scores_df
